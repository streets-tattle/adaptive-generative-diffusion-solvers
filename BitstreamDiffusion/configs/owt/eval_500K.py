...
def get_config():
    ...
    # ------------------------------------------------------------------
    # Evaluation checkpoint / output directories
    # ------------------------------------------------------------------
    ckpt_name = "step=000500000.pt"
    ckpt_label = "500K"

    cfg.evaluation = config_dict.ConfigDict()
    cfg.evaluation.checkpoint_path = (
        f"runs/{cfg.experiment}/checkpoints/{ckpt_name}"
    )

    # Output directory named for Config A
    cfg.evaluation.out_dir = (
        f"runs/{cfg.experiment}/evaluation_external_ppl_{ckpt_label}_pareto_nfe256_ext"
    )
    cfg.evaluation.samples_dir = (
        f"{cfg.evaluation.out_dir}/samples"
    )
    cfg.evaluation.results_csv = (
        f"{cfg.evaluation.out_dir}/results.csv"
    )
    cfg.evaluation.shared_text_cache_dir = (
        f"{cfg.evaluation.out_dir}/shared_text_cache"
    )
    ...
    # ------------------------------------------------------------------
    # Pareto sampling sweep: Config A (NFE 256 Extension)
    # ------------------------------------------------------------------
    ...
    target_nfes = [256]
    eta_grid = [0.0, 0.3, 0.6, 0.9]

    gamma_grid_by_nfe = {
        256: [0.11, 0.13, 0.15, 0.17, 0.19, 0.22, 0.25, 0.28],
    }

    # Deterministic anchor: one per NFE, eta=0 only.
    ...
    # Stochastic Pareto sweep.
    for target_nfe in target_nfes:
        for gamma_target in gamma_grid_by_nfe[target_nfe]:
            for eta in eta_grid:
                add_spec(
                    target_nfe=target_nfe,
                    ati_eta=eta,
    ...
    expected = len(target_nfes) + len(gamma_grid_by_nfe[256]) * len(eta_grid)
    actual = len(cfg.evaluation.sampling_sweep.specs)

    print(
        f"[sampling_sweep:{ckpt_label}] specs = {actual} "
        f"({len(target_nfes)} deterministic + "
        f"{len(gamma_grid_by_nfe[256])} gammas x {len(eta_grid)} etas)"
    )
    if actual != expected:
        raise ValueError(f"Unexpected number of sweep specs: actual={actual}, expected={expected}")
