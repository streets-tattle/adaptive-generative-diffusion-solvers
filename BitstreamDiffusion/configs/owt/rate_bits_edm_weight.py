#configs/owt/rate_bits_edm_weight.py
...
def get_config():
    ...
    cfg.model.patch_size = 16  # 1 code token = 16 bits
    ...
    # Debug only for the double-tokenized OWT setup.
    ...
    # Disable unrelated metrics for now.
    ...
    # Pick a modest size first for the correction check.
    cfg.evaluation.external_ppl.num_samples = 1024
    cfg.evaluation.external_ppl.micro_batch_size = 128

    # Single evaluation setting:
    # DDIM + entropic spacing + carry SC refresh + no ATI.
    ...
    # Keep the decode debug on for the double-tokenized OWT setup.
