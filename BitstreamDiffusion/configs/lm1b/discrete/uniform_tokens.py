...
# LM1B unconditional discrete uniform-tokens baseline
# Matched as closely as possible to the absorb-tokens baseline.
# The only intended differences are those required by the diffusion process:
# uniform-state process vs absorbing masked process.
...
def get_config():
    ...
    cfg.experiment = "paper/unconditional_text/lm1b/uniform_tokens_matched_121M_33B"
    cfg.device = "cuda"

    # ------------------------------------------------------------------
    # Data: LM1B tokens on semantic-rank ids
    # ------------------------------------------------------------------
    ...
    # Uniform diffusion lives on the base vocabulary only.
    ...
    # ------------------------------------------------------------------
    # Model
    # Match absorb-tokens baseline exactly except the uniform-specific
    # recommendation scale_by_sigma=False.
    # ------------------------------------------------------------------
    ...
    # One LM token = one trunk token in token-space discrete runs.
    cfg.model.patch_size = 1

    # Strong benchmark-facing trunk
    ...
    # ------------------------------------------------------------------
    # Discrete diffusion: uniform-state process
    # Only this block should differ materially from absorb-tokens
    # ------------------------------------------------------------------
    ...
    # Match absorb-tokens baseline exactly
    # ------------------------------------------------------------------
    ...
    cfg.logging.group = "lm1b_uniform_tokens_matched_121m_33b"
