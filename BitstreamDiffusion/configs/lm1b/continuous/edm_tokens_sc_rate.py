...
# LM1B continuous baseline on one-hot token vectors in R^V.
# ADAPTIVE RATE SCHEDULE + UNIT LOSS WEIGHTING + SELF-CONDITIONING
...
def get_config():
    ...
    cfg.experiment = "paper/unconditional_text/lm1b/continuous_onehot_tokens_sc_rate"
    cfg.device = "cuda"
    ...
    # Base proposal distribution before adaptive kicks in
    cfg.train.sigma_sampling_strategy = "log-normal"
    cfg.train.self_condition_prob = 0.5

    # --- ONLINE ENTROPY SCHEDULE CONFIG ---
    ...
    cfg.train.entropy_target = "rate" # <--- RATE (exponent 1.0)
