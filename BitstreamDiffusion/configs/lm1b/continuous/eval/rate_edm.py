...
# LM1B unconditional continuous rate-sampled raw-binary model with EDM weighting.
# Evaluation-only config:
#   - checkpoints: 100K, 200K, ..., 900K
#   - metric: external PPL
...
def get_config():
    ...
    # Isolation ablation: entropy-driven sigma sampling retained, EDM weighting.
    ...
    cfg.train.external_ppl.micro_batch_size = 512
    ...
    # Must exist because run_eval.py loads this before the external-PPL driver.
    cfg.evaluation.checkpoint_path = (
        f"runs/{cfg.experiment}/checkpoints/step=000900000.pt"
    )

    cfg.evaluation.out_dir = (
        f"runs/{cfg.experiment}/external_ppl_ckpt100k_900k_nfe512"
    )
    ...
    # Safer for checkpoint sweeps because run_eval.py compiles before
    # evaluate_external_ppl reloads checkpoints.
    ...
    cfg.evaluation.mauve.checkpoints = _ckpts_100k_to_1000k()
    ...
    cfg.evaluation.external_ppl.checkpoints = _ckpts_100k_to_1000k()
