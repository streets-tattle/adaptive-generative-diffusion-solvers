...
# LM1B unconditional continuous rate-based raw-binary bitstream model.
# Evaluation-only config:
#   - checkpoint: 1M only
#   - metric: external PPL
#   - sampler: ddim_entropic
#   - sc_refresh_mode: carry
#   - target NFEs: 64, 128, 256, 512
#   - ATI eta: 0.0 and 0.5
#   - stochastic churn: sweep via s_churns
#   - terminal_sigma: 0.08
#   - score_mode: full


def _ckpts_1000k_only():
    return [
        "step=001000000.pt",
    ]
...
def get_config():
    ...
    cfg.evaluation.checkpoint_path = (
        f"runs/{cfg.experiment}/checkpoints/step=001000000.pt"
    )

    # Fresh directory to avoid mixing with old stoch-ch0 cache/results.
    cfg.evaluation.out_dir = (
        f"runs/{cfg.experiment}/"
        f"evaluation_1M_stochastic_fixed_ati"
    )
    ...
    # ------------------------------------------------------------------
    # ATI defaults.
    # Actual sweep values are controlled by sampling_sweep.specs.
    # ------------------------------------------------------------------
    cfg.evaluation.ati = config_dict.ConfigDict()
    cfg.evaluation.ati.enabled = True
    cfg.evaluation.ati.eta = 0.5

    # ------------------------------------------------------------------
    # Stochastic churn defaults.
    # Actual per-run values are controlled by sampling_sweep.specs.
    # ------------------------------------------------------------------
    ...
    # Target: find the lowest GenPPL at entropy close to the real-data
    # token-unigram entropy band, not to maximize entropy.
    #
    # We add intermediate NFEs 96, 192, 384 and run eta=0.5 selectively
    # where ATI may improve the constrained GenPPL--entropy tradeoff.
    cfg.evaluation.sampling_sweep.target_nfes = [96, 128, 192, 256, 384, 512]

    cfg.evaluation.sampling_sweep.specs = [
        # --------------------------------------------------------------
        # Deterministic baselines for new intermediate NFEs.
        #
        # Existing deterministic points for 128/256/512 can be reused
        # if already in the same cache/results directory. We include
        # 96/192/384 for both eta values to place the new curves.
        # --------------------------------------------------------------
        config_dict.ConfigDict({
            "sampler_name": "ddim_entropic",
            "sc_refresh_modes": ["carry"],
            "target_nfes": [96, 192, 384],
            "ati_etas": [0.0, 0.5],
        ...
        # --------------------------------------------------------------
        # NFE 96:
        # Important bridge between 64 and 128.
        #
        # Goal: find the lowest GenPPL at entropy close to the real-data
        # band around 4.31. Use both eta values and a denser moderate-
        # churn grid.
        # --------------------------------------------------------------
        config_dict.ConfigDict({
            "sampler_name": "ddim_entropic",
            ...
            "s_noise": 1.003,
            "s_churns": [3.0, 4.5, 6.0, 7.5, 9.0, 11.0, 14.0, 18.0],
        }),

        # --------------------------------------------------------------
        # NFE 128:
        # Moderate churn only. Higher churn moves entropy too high and
        # tends to worsen GenPPL.
        # --------------------------------------------------------------
        config_dict.ConfigDict({
            "sampler_name": "ddim_entropic",
            "sc_refresh_modes": ["carry"],
            ...
            "stochastic_enabled": True,
            "s_noise": 1.003,
            "s_churns": [6.0, 8.0, 10.0, 12.0],
        }),

        # --------------------------------------------------------------
        # NFE 192:
        # Key bridge region between 128 and 256. This may become one of
        # the best low-GenPPL / near-real-entropy tradeoff regions.
        # --------------------------------------------------------------
        config_dict.ConfigDict({
            "sampler_name": "ddim_entropic",
            "sc_refresh_modes": ["carry"],
            ...
            "stochastic_enabled": True,
            "s_noise": 1.003,
            "s_churns": [12.0, 18.0, 24.0, 32.0, 42.0],
        }),

        # --------------------------------------------------------------
        # NFE 256:
        # Main target region. Existing eta=0.5, ch=30/40 looked very
        # strong around entropy ≈ 4.31, so refine around that band.
        # --------------------------------------------------------------
        config_dict.ConfigDict({
            "sampler_name": "ddim_entropic",
            "sc_refresh_modes": ["carry"],
            ...
            "stochastic_enabled": True,
            "s_noise": 1.003,
            "s_churns": [24.0, 28.0, 32.0, 36.0, 42.0, 50.0, 58.0],
        }),

        # --------------------------------------------------------------
        # NFE 384:
        # Potential high-NFE compromise. Avoid very aggressive churn
        # because 512-NFE stochastic points already showed entropy
        # collapse under large churn.
        # --------------------------------------------------------------
        config_dict.ConfigDict({
            "sampler_name": "ddim_entropic",
            "sc_refresh_modes": ["carry"],
            ...
            "stochastic_enabled": True,
            "s_noise": 1.003,
            "s_churns": [4.0, 8.0, 12.0, 18.0, 26.0, 36.0],
        }),

        # --------------------------------------------------------------
        # NFE 512:
        # Tiny churn probes only. Previous ch=10 already reduced entropy
        # too much, so test whether very small churn can improve GenPPL
        # without moving too far from the target entropy band.
        # --------------------------------------------------------------
        config_dict.ConfigDict({
            "sampler_name": "ddim_entropic",
            "sc_refresh_modes": ["carry"],
    ...
    cfg.evaluation.mauve.checkpoints = _ckpts_1000k_only()
    ...
    cfg.evaluation.external_ppl.checkpoints = _ckpts_1000k_only()
